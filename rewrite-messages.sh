#!/bin/bash
# Git filter-branch to rewrite commit messages
# This preserves all dates and author information
...
git filter-branch -f --msg-filter '
    ...
    # Remove emojis and rewrite messages
    case "$msg" in
        "Change to scalar"*)
...
EOF
            ;;
        *"Implementazione completa ShopVerse"*)
            cat <<EOF
...
EOF
            ;;
        *"Implement comprehensive NotificationService"*)
            cat <<EOF
...
EOF
            ;;
        *"Implement comprehensive ChatbotService with AI integration"*)
            cat <<EOF
...
EOF
            ;;
        *"Delay for lazzy orderCreatedConsumer"*)
            cat <<EOF
...
' --env-filter 'export GIT_COMMITTER_DATE="$GIT_AUTHOR_DATE"' -- d7ee8b2..HEAD

echo ""
echo "Done! Commit messages have been rewritten while preserving dates."
echo ""
echo "To push: git push --force-with-lease origin dev"
